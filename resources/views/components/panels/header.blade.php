            ...
            <div class="flex justify-center">
                @if(request()->routeIs('home'))
                    <span class="inline-block sm:inline">
                        <img src="{{asset('assets/images/logo.png')}}" width="222" height="30" alt="">
                    </span>
                @else
            ...
            <div>
                @if(Auth::check())
                    <nav class="flex justify-center sm:justify-end items-center space-x-8 text-sm">
                        <a class="text-black hover:text-orange" href="{{route('dashboard')}}">
                        ...
                        <a class="text-gray-500 hover:text-orange" href="{{route('dashboard')}}">
                            <svg xmlns="http://www.w3.org/2000/svg" class="inline-block text-orange h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <a class="text-gray-500 hover:text-orange" href="{{route('logout')}}" onclick="event.preventDefault();this.closest('form').submit();">
                                <svg xmlns="http://www.w3.org/2000/svg" class="inline-block text-orange h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="border-b">
        <div class="container mx-auto block lg:flex justify-between items-center px-2 sm:px-0">
            ...
            @if(isset($category->id))
                <x-panels.menu :current="$category->id" />
            @else
                <x-panels.menu :current="0" />
            @endif
        </div>
    </div>
</header>
